...
const TaskDetails = ({navigation, route}: Props) => {
  ...
  const {task_title, task_Description, task_due_date, task_status} =
    useAppSelector(state => state.task.taskData);

  useEffect(() => {
    dispatch(fetchTaskDetailsAsync({task_id}));
  }, [task_id]);
